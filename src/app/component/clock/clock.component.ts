...
export class ClockComponent implements OnInit, AfterViewInit {
  ...
  @ViewChildren(FlipComponent) flipObjs: QueryList<FlipComponent>;
  ...
  init() {
    ...
    this.flipObjs['_results'].map((item, i) => {
      item.setFront(nowTimeStr[i]);
    });
  }
  // 开始计时
  run() {
    this.timer = setInterval(() => {
      ...
      for (let i = 0; i < this.flipObjs.length; i++) {
        ...
        this.flipObjs['_results'][i].flipDown(
          nowTimeStr[i],
          nextTimeStr[i]
        );
      }
